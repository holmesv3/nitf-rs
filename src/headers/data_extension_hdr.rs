...
const DE_LENGTH: u8 = 2u8;
const DESID_LENGTH: u8 = 25u8;
const DESVER_LENGTH: u8 = 2u8;
const DESOFLW_LENGTH: u8 = 6u8;
const DESITEM_LENGTH: u8 = 3u8;
const DESSHL_LENGTH: u8 = 4u8;
...
impl NitfSegmentHeader for DataExtensionHeader {
    fn read(&mut self, reader: &mut File) -> NitfResult<()> {
        self.de.read(reader, DE_LENGTH, "DE")?;
        self.desid.read(reader, DESID_LENGTH, "DESID")?;
        self.desver.read(reader, DESVER_LENGTH, "DESVER")?;
        self.security.read(reader)?;
        if self.desid.string().trim() == "TRE_OVERFLOW" {
            self.desoflw.read(reader, DESOFLW_LENGTH, "DESOFLW")?;
            self.desitem.read(reader, DESITEM_LENGTH, "DESITEM")?;
        }
        self.desshl.read(reader, DESSHL_LENGTH, "DESSHL")?;
